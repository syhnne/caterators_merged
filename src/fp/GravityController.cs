...
public class GravityController_v2
{
    public GravityController_v2(Player player)
    {
        owner = new(player);
    }
    ...
    public Room room => player != null? player.room : null;

    public int gravity = 10;
    public bool IsActive => enabled && player != null && room != null && room.abstractRoom.name.StartsWith("SS") && room.abstractRoom.name != "SS_D08" && room.abstractRoom.name != "SS_E08" && room.game.IsStorySession && room.game.StoryCharacter == player.SlugCatClass;
    public bool KeyPressed => IsActive && player.Consious && !player.dead && player.stun == 0 && Input.GetKey(Plugin.instance.configOptions.GravityControlKey.Value);

    private bool enabled = true;
    public int pressedCounter = 0;
    public int inputY = 0;
    public WeakReference<AntiGravity> antiG;

    public static readonly int Speed = 12;
    public static readonly int MaxGravity = 10;
    ...
    public void Update(bool eu)
    {
        ...
        if (!IsActive)
        {
            ...
            gravity = (int)Mathf.Round(10f * player.room.gravity);
        }
        else if (KeyPressed)
        {
            player.Blink(5);
            ...
            {
                pressedCounter++;
                if (pressedCounter >= Speed)
                {
                    pressedCounter = 0;
                    if ((gravity + inputY) >= 0 && (gravity + inputY) <= MaxGravity)
                    {
                        gravity += inputY;
                        room.gravity = gravity * 0.1f;
                    }
                }
            }
        }
    ...
    public void NewRoom(Room newRoom)
    {
        ...
        var roomSettings = player.room.roomSettings;
        if (roomSettings.GetEffect(RoomSettings.RoomEffect.Type.ZeroG) != null || roomSettings.GetEffect(RoomSettings.RoomEffect.Type.BrokenZeroG) != null)
        {
            foreach (var obj in room.updateList)
            {
                if (obj is AntiGravity)
                {
                    antiG = new(obj as AntiGravity);
                    (obj as AntiGravity).active = false;
                    break;
                }
            }
        }
...
public class GravityMeter_v2 : HudPart
{
    private GravityController_v2 owner;
    public Vector2 pos;
    ...
    public GravityMeter_v2(HUD.HUD hud, FContainer fContainer, GravityController_v2 owner) : base(hud)
    {
    ...
    private bool Show
    {
        get
        {
            return owner != null && owner.room != null && owner.IsActive && owner.KeyPressed;
        }
    }
    ...
    public override void Update()
    {
        ...
        for (int i = 0; i < circles.Length; i++)
        {
            circles[i].Update();
            circles[i].fade = fade;
            circles[i].pos = pos + new Vector2(i * 21.6f, 0f);
            circles[i].visible = true;
            circles[i].pos = pos + Custom.DegToVec((1f - i / (float)circles.Length) * 360f * Custom.SCurve(Mathf.Pow(fade, 1.5f - i / (float)(circles.Length - 1)), 0.6f)) * 32f;
        }
...
public class GravityController_v1 : UpdatableAndDeletable
{
    public Player player;
    ...
    public GravityController_v1(Player player)
    {
        this.player = player;
        unlocked = player.room.game.IsStorySession && player.room.game.StoryCharacter == Enums.FPname;
...
public class GravityMeter_v1 : HudPart
{
    private GravityController_v1 owner;
    public Vector2 pos;
    public Vector2 lastPos;
    public float fade;
    public float lastFade;
    ...
    public GravityMeter_v1(HUD.HUD hud, FContainer fContainer, GravityController_v1 owner) : base(hud)
    {
        circles = new HUDCircle[10];
